...
class Batch {
    ...
    remove(id, assinanteId, tipo) {
        const targetId = id.id || id;
        const targetAssinanteId = id.assinanteId || assinanteId;
        const targetTipo = id.tipo || tipo;
        const index = this._items.findIndex(item => 
            item.id === targetId && 
            (targetAssinanteId ? item.assinanteId === targetAssinanteId : true) && 
            (targetTipo ? item.tipo === targetTipo : true));
        if (index !== -1)
